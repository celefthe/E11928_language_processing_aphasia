...
for subject = 1:size(subj,1)
    cd(bids_dir); % go back to the bids_dir at beginning of every iteration
    if strcmp(subj(subject).name(5:11),'healthy')
        ...
        ThreeD_names = spm_file_split(spm_vol(cell2mat(fMRIname)));
        ...
        spmup_smooth_boostedfiles(boosted_files{1}, [6 6 6]);
